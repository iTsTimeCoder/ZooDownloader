...
std::string DetectPlatform(const std::string & url) {
    ...
    if (u.find("youtu.be") != std::string::npos || u.find("youtube.com") != std::string::npos || u.find("music.youtube.com") != std::string::npos) return "YouTube";
    if (u.find("tiktok.com") != std::string::npos) return "TikTok";
    if (u.find("instagram.com") != std::string::npos) return "Instagram";
    if (u.find("kick.com") != std::string::npos) return "Kick";
    if (u.find("twitch.tv") != std::string::npos) return "Twitch";
    if (u.find("twitter.com") != std::string::npos || u.find("x.com") != std::string::npos) return "Twitter / X";
    if (u.find("facebook.com") != std::string::npos || u.find("fb.watch") != std::string::npos) return "Facebook";
    if (u.find("reddit.com") != std::string::npos || u.find("v.redd.it") != std::string::npos) return "Reddit";
    if (u.find("soundcloud.com") != std::string::npos) return "SoundCloud";
    if (u.find("spotify.com") != std::string::npos) return "Spotify";
    if (u.find("dailymotion.com") != std::string::npos) return "Dailymotion";
    if (u.find("vimeo.com") != std::string::npos) return "Vimeo";
    if (u.find("bilibili.com") != std::string::npos) return "Bilibili";
    if (u.find("rumble.com") != std::string::npos) return "Rumble";
    if (u.find("pinterest.com") != std::string::npos) return "Pinterest";
    if (u.find("linkedin.com") != std::string::npos) return "LinkedIn";
    if (u.find("snapchat.com") != std::string::npos) return "Snapchat";
    if (u.find("odysee.com") != std::string::npos) return "Odysee";
    if (u.find("trovo.live") != std::string::npos) return "Trovo";
    if (u.find("streamable.com") != std::string::npos) return "Streamable";
    if (u.find("bandcamp.com") != std::string::npos) return "Bandcamp";
    if (u.find("mixcloud.com") != std::string::npos) return "Mixcloud";
    if (u.find("nicovideo.jp") != std::string::npos || u.find("niconico.jp") != std::string::npos) return "NicoNico";
    if (u.find("aparat.com") != std::string::npos) return "Aparat";
    if (u.find("t.me") != std::string::npos) return "Telegram";
    if (u.find("threads.net") != std::string::npos) return "Threads";
    if (u.find("bbc.co.uk") != std::string::npos) return "BBC";
    if (u.find("cnn.com") != std::string::npos) return "CNN";
    if (u.find("crunchyroll.com") != std::string::npos) return "Crunchyroll";
    if (u.find("funimation.com") != std::string::npos) return "Funimation";
    if (u.find("naver.com") != std::string::npos) return "Naver TV";
    if (u.find("weibo.com") != std::string::npos) return "Weibo";
    if (u.find("coub.com") != std::string::npos) return "Coub";
    if (u.find("gfycat.com") != std::string::npos) return "Gfycat";
    if (u.find("brighteon.com") != std::string::npos) return "Brighteon";
    return u.find("http") != std::string::npos ? "Auto-Detect (yt-dlp)" : "---";
}

void EnsureExecutables() {
    if (GetFileAttributesA("yt-dlp.exe") == INVALID_FILE_ATTRIBUTES) {
        URLDownloadToFileA(NULL, "https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp.exe", "yt-dlp.exe", 0, NULL);
    }
    if (GetFileAttributesA("ffmpeg.exe") == INVALID_FILE_ATTRIBUTES) {
        std::string psCmd = "-WindowStyle Hidden -Command \"Invoke-WebRequest -Uri 'https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/ffmpeg-master-latest-win64-gpl.zip' -OutFile 'ffmpeg.zip'; Expand-Archive -Path 'ffmpeg.zip' -DestinationPath 'ffmpeg_temp' -Force; Copy-Item 'ffmpeg_temp\\ffmpeg-master-latest-win64-gpl\\bin\\ffmpeg.exe' -Destination 'ffmpeg.exe' -Force; Remove-Item 'ffmpeg.zip' -Force; Remove-Item 'ffmpeg_temp' -Recurse -Force\"";
        ShellExecuteA(NULL, "open", "powershell.exe", psCmd.c_str(), NULL, SW_HIDE);
    }
}

void AutoUpdate() {
    if (GetFileAttributesA("yt-dlp.exe") != INVALID_FILE_ATTRIBUTES) {
        ShellExecuteA(NULL, "open", "cmd.exe", "/c yt-dlp.exe -U > nul 2>&1", NULL, SW_HIDE);
    }
}
...
HWND MkLabel(HWND p, const char* t, int x, int y, int w, int h, HFONT f) {
    HWND hw = CreateWindowExA(0, "STATIC", t, WS_VISIBLE|WS_CHILD, x, y, w, h, p, NULL, g_hInst, NULL);
    SF(hw, f); return hw;
}

HWND MkEdit(HWND p, const char* def, int x, int y, int w, int h, int id, DWORD ex) {
    HWND hw = CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", def, WS_VISIBLE|WS_CHILD|WS_TABSTOP|ES_AUTOHSCROLL|ex, x, y, w, h, p, (HMENU)(INT_PTR)id, g_hInst, NULL);
    SF(hw, nullptr); return hw;
}

HWND MkMultiEdit(HWND p, int x, int y, int w, int h, int id) {
    HWND hw = CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", "", WS_VISIBLE|WS_CHILD|WS_TABSTOP|WS_VSCROLL|ES_MULTILINE|ES_AUTOVSCROLL|ES_WANTRETURN, x, y, w, h, p, (HMENU)(INT_PTR)id, g_hInst, NULL);
    SF(hw, nullptr); return hw;
}

HWND MkCheck(HWND p, const char* t, int x, int y, int w, int h, int id, bool chk) {
    HWND hw = CreateWindowExA(0, "BUTTON", t, WS_VISIBLE|WS_CHILD|BS_AUTOCHECKBOX|WS_TABSTOP, x, y, w, h, p, (HMENU)(INT_PTR)id, g_hInst, NULL);
    SF(hw, nullptr);
...
HWND MkRadio(HWND p, const char* t, int x, int y, int w, int h, int id, DWORD ex) {
    HWND hw = CreateWindowExA(0, "BUTTON", t, WS_VISIBLE|WS_CHILD|BS_AUTORADIOBUTTON|WS_TABSTOP|ex, x, y, w, h, p, (HMENU)(INT_PTR)id, g_hInst, NULL);
    SF(hw, nullptr); return hw;
}

HWND MkCombo(HWND p, int x, int y, int w, int h, int id) {
    HWND hw = CreateWindowExA(0, "COMBOBOX", "", WS_VISIBLE|WS_CHILD|CBS_DROPDOWNLIST|WS_TABSTOP, x, y, w, h+200, p, (HMENU)(INT_PTR)id, g_hInst, NULL);
    SF(hw, nullptr); return hw;
}

HWND MkBtn(HWND p, const char* t, int x, int y, int w, int h, int id, DWORD ex) {
    HWND hw = CreateWindowExA(0, "BUTTON", t, WS_VISIBLE|WS_CHILD|BS_PUSHBUTTON|WS_TABSTOP|ex, x, y, w, h, p, (HMENU)(INT_PTR)id, g_hInst, NULL);
    SF(hw, g_fBold); return hw;
}

void ComboFill(HWND hw, const char* items[], int n, int sel) {
    for (int i = 0; i < n; i++) SendMessageA(hw, CB_ADDSTRING, 0, (LPARAM)items[i]);
...
void CreateDownloadPanel() {
    ...
    const char* vq[] = { "Best Available", "4K (2160p)", "1440p (2K)", "1080p", "720p", "480p", "360p", "240p", "Worst" };
    ...
    MkLabel(p, "(Use cookies for age-restricted / private / login-required content)", 215, 293, 510, 17, g_fSmall);
    MkLabel(p, "Supported Platforms:", 10, 328, 155, 18, g_fBold);
    MkLabel(p, "YouTube | TikTok | Instagram | Kick | Twitch | Twitter/X | Facebook | Reddit | SoundCloud | Vimeo | Bilibili | Rumble | Odysee | Bandcamp | Dailymotion | Mixcloud | Spotify | Pinterest | NicoNico | Aparat | Snapchat | and 1000+ more!", 10, 348, 728, 48, g_fSmall);
}
...
void CreateAdvancedPanel() {
    ...
    const char* sl[] = { "en", "ar", "fr", "de", "es", "ja", "zh-Hans", "zh-Hant", "ko", "ru", "pt", "it", "tr", "pl", "hi", "id", "nl", "sv", "da", "no" };
    ...
    y += 34;
    g_hAria2 = MkCheck(p, "Use aria2c downloader (faster, needs aria2c.exe in path)", 10, y, 430, 22, IDC_CHK_ARIA2);
    y += 38;
    MkLabel(p, "Output Filename Template:", 10, y, 220, 18, g_fLarge); y += 26;
    g_hFilenameTmpl = MkEdit(p, "%(uploader)s - %(title)s.%(ext)s", 10, y, 728, 26, IDC_EDT_FILENAME);
    MkLabel(p, "Playlist: %(playlist_index)03d. %(title)s.%(ext)s | Variables: %(title)s %(uploader)s %(id)s %(ext)s %(upload_date)s", 10, y+30, 728, 17, g_fSmall);
    y += 58;
...
void CreateAboutPanel() {
    ...
    y += 14;
    MkLabel(p, "Requirements: yt-dlp.exe + ffmpeg.exe (will be auto-downloaded if missing)", 18, y, 718, 18, g_fSmall); y += 32;
    MkBtn(p, "  Update yt-dlp Engine", 18, y, 200, 36, IDC_BTN_UPDATE);
}

void ShowTab(int idx) {
    for (int i = 0; i < 3; i++) ShowWindow(g_hPanels[i], i == idx ? SW_SHOW : SW_HIDE);
}
...
void BuildAndRun(HWND hwnd) {
    ...
    if (isAud) {
        ...
        if (h > 0) ss << "-f \"bestvideo[height<=" << h << "]+bestaudio/best[height<=" << h << "]\" ";
        else if (q == "Worst") ss << "-f \"worstvideo+worstaudio/worst\" ";
        else ss << "-f \"bestvideo+bestaudio/best\" ";
    ...
    if (Chk(g_hAria2)) ss << "--downloader aria2c --downloader-args \"-c -j8 -s8 -x8 -k1M\" ";
    if (Chk(g_hSponsorBlock)) ss << "--sponsorblock-remove sponsor,selfpromo,interaction,intro,outro ";
    if (Chk(g_hSubs)) {
    ...
    if (!conc.empty() && conc != "1" && conc != "0") ss << "--concurrent-fragments " << conc << " ";
    std::string custom = GetWndText(g_hCustomArgs);
    if (!custom.empty()) ss << custom << " ";
    std::string tmpl = GetWndText(g_hFilenameTmpl);
    if (Chk(g_hPlaylist)) {
        ss << "--yes-playlist ";
        if (!tmpl.empty()) ss << "-o \"%(playlist_index)03d. " << tmpl << "\" ";
        else ss << "-o \"%(playlist_index)03d. %(title)s.%(ext)s\" ";
    } else {
        ss << "--no-playlist ";
        if (!tmpl.empty()) ss << "-o \"" << tmpl << "\" ";
        else ss << "-o \"%(uploader)s - %(title)s.%(ext)s\" ";
    }
    ss << "--newline ";
    if (isBatch) {
    ...
    AppendLog("  Mode     : " + std::string(isAud ? "Audio (" + GetComboSel(g_hAudFmt) + ")" : "Video (" + GetComboSel(g_hVidQual) + ")"));
    ...
    AppendLog("-----------------------------------------");
    std::string shellArgs = "/c yt-dlp " + ss.str() + " & echo. & echo ================================================= & echo ZooDownloader - Download Complete! & echo ================================================= & pause";
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_CREATE: {
        ...
        INITCOMMONCONTROLSEX icc = {sizeof(icc), ICC_TAB_CLASSES|ICC_PROGRESS_CLASS|ICC_STANDARD_CLASSES};
        InitCommonControlsEx(&icc);
        g_hTab = CreateWindowExA(0, WC_TABCONTROLA, "", WS_VISIBLE|WS_CHILD|TCS_HOTTRACK, 5, 5, 754, 430, hwnd, (HMENU)IDC_TAB, g_hInst, NULL);
        SF(g_hTab, g_fBold);
        ...
        for (int i = 0; i < 3; i++) {
            g_hPanels[i] = CreateWindowExA(0, "ZooPanel", "", WS_CHILD | (i == 0 ? WS_VISIBLE : 0), r.left, r.top, r.right - r.left, r.bottom - r.top, hwnd, NULL, g_hInst, NULL);
        }
        CreateDownloadPanel();
        CreateAdvancedPanel();
        CreateAboutPanel();
        ...
        g_hLog = CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", "", WS_VISIBLE|WS_CHILD|ES_MULTILINE|ES_READONLY|WS_VSCROLL|ES_AUTOVSCROLL, 5, 462, 754, 110, hwnd, (HMENU)IDC_LOG_OUTPUT, g_hInst, NULL);
        SF(g_hLog, g_fMono);
        ...
        g_hProgress = CreateWindowExA(0, PROGRESS_CLASS, "", WS_VISIBLE|WS_CHILD|PBS_SMOOTH|PBS_MARQUEE, 604, 578, 155, 50, hwnd, (HMENU)IDC_PROGRESS, g_hInst, NULL);
    ...
    case WM_NOTIFY: {
        NMHDR* nm = (NMHDR*)lp;
        if (nm->idFrom == IDC_TAB && nm->code == TCN_SELCHANGE) ShowTab(TabCtrl_GetCurSel(g_hTab));
        break;
    }
    case WM_COMMAND: {
        int id = LOWORD(wp);
        ...
        case IDC_BTN_STOP:
            ...
            break;
        case IDC_BTN_UPDATE:
            ShellExecuteA(NULL, "open", "cmd.exe", "/c yt-dlp -U & echo. & echo Update complete! & pause", NULL, SW_SHOW);
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, int nShow) {
    ...
    RegisterClassExA(&wc);
    HWND hwnd = CreateWindowExA(WS_EX_APPWINDOW, "ZooDownloader_v1", "ZooDownloader v1.0 -- By iTsTimeCoder", WS_OVERLAPPEDWINDOW & ~WS_THICKFRAME & ~WS_MAXIMIZEBOX, CW_USEDEFAULT, CW_USEDEFAULT, 776, 680, NULL, NULL, hInst, NULL);
